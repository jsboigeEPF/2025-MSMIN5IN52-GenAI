...
class BiasEvaluationDashboard {
    ...
    updateModelDetailsTable() {
        ...
        const modelData = this.filters.selectedModels.map(model => {
            ...
            const socioeconomicBias = result.socioeconomic_bias ? result.socioeconomic_bias.bias_score || 0 : 0;
            const sexualOrientationBias = result.sexual_orientation_bias ? result.sexual_orientation_bias.bias_score || 0 : 0;
            const toxicity = result.toxicity ? result.toxicity.bias_score || 0 : 0;
            const globalScore = (genderBias + racialBias + socioeconomicBias + sexualOrientationBias + toxicity) / 5;
        ...
        modelData.forEach((data, index) => {
            ...
            row.innerHTML = `
                ...
                <td><span class="${getBiasClass(data.socioeconomicBias)}">${formatScore(data.socioeconomicBias)}</span></td>
                <td><span class="${getBiasClass(data.sexualOrientationBias)}">${formatScore(data.sexualOrientationBias)}</span></td>
