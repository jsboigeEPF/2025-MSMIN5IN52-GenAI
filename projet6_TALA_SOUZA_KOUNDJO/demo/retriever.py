...
class ApplicantID(BaseModel):
  """
  Liste des IDs des candidats pour lesquels récupérer les CV
  """
  id_list: List[str] = Field(..., description="Liste des IDs des candidats pour lesquels récupérer les CV")

class JobDescription(BaseModel):
  """
  Description du poste pour récupérer les CV similaires
  """
  job_description: str = Field(..., description="Description du poste pour récupérer les CV similaires") 
...
class RAGRetriever():
  ...
  def retrieve_documents_with_id(self, doc_id_with_score: dict, threshold=5):
    ...
    for i in range(len(retrieved_documents)):
      retrieved_documents[i] = "ID Candidat " + retrieved_ids[i] + "\n" + retrieved_documents[i]
...
class SelfQueryRetriever(RAGRetriever):
  def __init__(self, vectorstore_db, df):
    ...
    self.prompt = ChatPromptTemplate.from_messages([
      ("system", "Tu es un expert en acquisition de talents."),
      ("user", "{input}")
    ])
    self.meta_data = {
      "rag_mode": "",
  ...
  def retrieve_docs(self, question: str, llm, rag_mode: str):
    ...
    def retrieve_applicant_id(id_list: list):
      """Récupérer les CV des candidats présents dans id_list"""
      retrieved_resumes = []

      for id in id_list:
        try:
          resume_df = self.df[self.df["ID"].astype(str) == id].iloc[0][["ID", "Resume"]]
          resume_with_id = "ID Candidat " + resume_df["ID"].astype(str) + "\n" + resume_df["Resume"]
    ...
    def retrieve_applicant_jd(job_description: str):
      """Récupérer les CV similaires correspondant à une description de poste"""
